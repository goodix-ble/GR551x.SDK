...
/// Alias
#define bb_watch_timer_start_dtm  bb_watch_timer_start_patch
#define bb_watch_timer_cbk_dtm  bb_watch_timer_cbk_patch
...
#define EM_BLE_RXCCMPKTCNT0_ADDR     (REG_EM_BASE_ADDR + 0x00000056 + EM_BLE_CS_OFFSET)

/// TXRXCNTL register definition
#define EM_BLE_TXRXCNTL_ADDR         (REG_EM_BASE_ADDR + 0x00000020 + EM_BLE_CS_OFFSET)
...
struct rwip_rf_api
{
    ...
    void (*sleep)(void);
    void (*default_tx_power_set)(int8_t);
    int8_t (*default_tx_power_get)();
    ...
    /// Function called to change tx mode
    uint8_t (*tx_mode_set)(uint8_t);
    /// Function called to get tx mode
    uint8_t (*tx_mode_get)(void);
    /// Index of rf match circuit (ohm)
    uint8_t mat_cir;
    /// Function called to set rf match circuit (ohm)
    void (*mat_cir_set)(uint8_t);
    /// Function called to get rf match circuit (ohm)
    uint8_t (*mat_cir_get)(void);
};
...
extern void lld_test_rx_isr_patch(uint32_t timestamp);
